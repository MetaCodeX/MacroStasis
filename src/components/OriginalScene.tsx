...
export function OriginalScene() {
  ...
  useEffect(() => {
    ...
    function renderMobile(gltf: any): THREE.Sprite[] {
      ...
      const hw = size.x / 2
      const my = center.y
      const modelTop = my + size.y / 2
      const modelBottom = my - size.y / 2
      const textH = size.y * 0.30
      const ch = 128 * 1.4

      const sprites: THREE.Sprite[] = []
      const gapY = textH * 0.12
      const gapX = textH * 0.35

      // ---- META stack (4 letters, left of model, above) ----
      ...
      const metaW = Math.max(...metaSprites.map(s => s.scale.x))
      const metaH = metaSprites[0].scale.y

      const metaLeftEdge = -hw - metaW - gapX

      let yPos = modelTop + gapY + metaH / 2
      for (const s of metaSprites) {
        placeLeft(s, metaLeftEdge, yPos)
        scene.add(s)
        sprites.push(s)
        yPos -= metaH + gapY
      }

      // ---- NAME stack (4 words, right of META, above model) ----
      const nameWords = ["CARLOS", "EDUARDO", "JUAREZ", "RICARDO"]
      const nameWordH = textH * 0.35
      const nameSprites = nameWords.map(w =>
        makeMetallicSprite(w, nameWordH, dpr, silver.c1, silver.c2, silver.hl, silver.sp)
      )
      const nameLeftEdge = metaLeftEdge + metaW + gapX

      yPos = modelTop + gapY + nameWordH / 2
      for (const s of nameSprites) {
        placeLeft(s, nameLeftEdge, yPos)
        scene.add(s)
        sprites.push(s)
        yPos -= nameWordH + gapY
      }

      // ---- CODEX stack (5 letters, right of model, below) ----
      ...
      const codexW = Math.max(...codexSprites.map(s => s.scale.x))
      const codexLeftEdge = hw + gapX

      yPos = modelBottom - gapY - metaH / 2
      for (const s of codexSprites) {
        placeLeft(s, codexLeftEdge, yPos)
        scene.add(s)
        sprites.push(s)
        yPos -= metaH + gapY
      }

      // ---- TITLE stack (4 lines, left of CODEX, right-aligned, below model) ----
      const titleLines = [
        "MID LEVEL",
        "FULL STACK ENGINEER",
        "| AI & AUTOMATION",
      ...
      yPos = modelBottom - gapY - titleLineH / 2
      for (const s of titleSprites) {
        placeRight(s, codexLeftEdge, yPos)
        scene.add(s)
        sprites.push(s)
        yPos -= titleLineH + gapY * 0.8
      }
    ...
    function onResize() {
      ...
      if (mobile !== (mode.current === "mobile")) {
        ...
        // Recreate bloom pass for mobile/desktop
